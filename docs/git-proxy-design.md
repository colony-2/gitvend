...
| Storage | Stateless gateway: configured verifier keys and credentials, per-request memory and structured audit output. No database, writable state volume, persistent quotas or repository-ID pinning. |
...
| R10 | Bound memory, request size, per-instance concurrency, and request duration. Stream large packs with backpressure. Fleet-wide rate limits and creation budgets are external concerns. |
...
Repository grants follow canonical owner/repository names, including names deleted and later recreated. Do not enroll repositories or pin provider IDs. Validate and activate local policy/configuration revisions atomically.
...
| Provisioning in `internal/gateway` | Authorized lookup/create and conflict reconciliation through the forge API, without local attempts or distributed locks. |
| `internal/audit` | Structured decision/outcome events on stdout; no local journal or outbox. |
...
All gateway instances operate independently from configuration, public verification keys and upstream credentials. Ordinary Git traffic has no REST lookup dependency; only creation-enabled push discovery uses the forge API. Concurrent creates rely on the forge's unique owner/name constraint, with a lookup to reconcile conflicts or ambiguous responses. There is no database, shared state volume, replica affinity or coordination service. Fleet-wide budgets and log storage belong outside the gateway.
...
The claim schema includes `iss`, `sub`, `aud`, `iat`, `nbf`, `exp`, and `jti`, plus a versioned `grant` containing the compact `permissions` string array, where leading `!` denotes a deny rule. `jti` is an audit identifier, not a database key or single-use nonce; Git uses the same JWT across multiple requests. All repository grants are name-scoped; the grant has no provider-ID binding fields. Include an issuer policy revision for audit, not as a reference the proxy must dereference. JWTs carry no upstream token, private key or other embedded secret: signed claims are readable by their holder. [JWT claims](https://www.rfc-editor.org/rfc/rfc7519.html)
...
Authorize every HTTP request again, including after discovery. Revalidate expiry after lookup and before starting a create operation, and again before forwarding Git traffic. An admitted transfer may finish under its verified grant/configuration snapshot, subject to a transfer deadline; expiry cannot undo an upstream write already accepted. Refresh ahead of multi-request Git operations; if a later request finds the token expired, the helper obtains the newly injected token and the client may need to retry. Record token `jti`, issuer policy revision and gateway configuration revision in audit.
...
1. Authorize the repository and ref operation before contacting the upstream. Only push discovery with configuration-enabled creation and a `c` grant enters the creation path.
2. Inspect the repository by name. An existing repository continues to Git discovery without changing its visibility. A failed lookup other than 404 fails the request. An opaque 404 may mean inaccessible; independent creation authority is required before attempting creation.
3. For a missing repository, require a nonempty readable/writable branch overlap and recheck JWT expiry. Emit a creation admission event.
4. Attempt one create with explicit private visibility and no initial commit. Clients cannot supply arbitrary templates, import URLs, visibility or credentials.
5. If create fails or its response is ambiguous, look up the name again. Confirm canonical owner/name and private visibility before continuing. A conflict is never a reason to modify an existing repository's settings. There is no blind retry of the create POST.
6. Emit `created` or `reconciled`, then request real Git discovery using transport credentials. Filter it by policy. Git transport failures remain failures; API success does not imply a successful push. Reauthorize the later push independently.

If reconciliation cannot confirm the repository, return 503 and log `unknown`. A later normal push discovery starts fresh; another replica or a restarted process can handle it. There are no stored attempts, creation quotas or background reconciliation jobs. The forge coordinates concurrent name creation, and a repository left empty after an abandoned/failed push is not automatically deleted.
...
| [`cmd/repo_create.go`](https://github.com/colony-2/c2r/blob/5a655c078e79c97c76caea0cbaec792013023e80/cmd/repo_create.go) | Lookup-before-create flow and creation inputs. | Extract service logic from Cobra; add cancellation, authorization and conflict reconciliation. |
...
Emit structured JSON audit events on stdout before upstream operations and after their outcomes. Record request/attempt ID, issuer, subject, token ID, canonical repository name, policy revision, actions, old/new object IDs, decision/reason, and final per-ref outcome. Creation records link to the parent Git request. Exclude JWT strings, upstream tokens, pack data and raw credential-bearing URLs. Log delivery is best-effort; there is no local persistence or transactional admission dependency. Collectors handle durability and retention. A crash may leave an admission without an outcome; inspect remote refs before retrying a push.

A future metrics endpoint could expose denies, request latency, active streams, bytes transferred, upstream authentication errors, credential reloads, rate limits, provisioning races, and unknown outcomes. Avoid unbounded principal/repository labels in metrics. Enforce configurable command-prefix/ref-count limits independently of pack limits, plus connection/idle deadlines and per-instance concurrency budgets. Large repositories should increase transfer time and bandwidth, not gateway memory proportional to pack size.
...
- Creation: absent repo, unauthorized name/owner, hidden 404, conflicts, concurrent first pushes through independent replicas, lost create response, restart recovery without local state, denied first branch, and documented dry-run creation.
...
3. Add stateless creation reconciliation, credential rotation, structured audit logs, resource limits, and GitHub end-to-end tests.
...
The Go implementation and automated tests are in this repository. The server is stateless and supports independent replicas without shared storage. Audit retention is external; metrics export remains follow-up work. Live forge account permissions require deployment-specific validation. See [README](../README.md) for the exact compatibility boundaries.
